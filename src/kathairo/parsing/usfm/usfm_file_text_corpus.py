...
class UsfmFileTextCorpus(ScriptureTextCorpus):
    def __init__(
        ...
        for sfm_filename in Path(project_dir).glob(file_pattern):
            texts.append(UsfmFileText(stylesheet, encoding, sfm_filename, handler, psalmSuperscriptionTag, include_headers, versification, include_markers)) #passes in handler
