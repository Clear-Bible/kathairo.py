...
import codecs
import pytest
from test import __tsv_vrs_files__
import pandas as pd
from machine.scripture import Versification

# Verify that the file exists.
@pytest.mark.parametrize("tsv_vrs_files", __tsv_vrs_files__)
...
@pytest.mark.parametrize("tsv_vrs_files", __tsv_vrs_files__)
def test_mapped_verses_are_present(tsv_vrs_files):
...
def test_chapter_size(tsv_vrs_files):
    ...
    for index in range(len(targetVersification.book_list)):
        assert (book_list[index] == targetVersification.book_list[index]), "Book Id: " + str(index + 1)