...
#from machine.corpora.usfm_parser import UsfmParser
            ...
            if self.handler is not None:
                ...
                for part in _OPT_BREAK_SPLITTER.split(text):
                    if part == "//":
                        ...
                        # TODO added by Robertson Brinker
                        if(self.state.prev_token is not None and self.state.prev_token.marker == "d" and self.state.verse_ref.book == "PSA"):
                            if self.handler is not None:
                                self.handler.verse(self.state, 0, "v", 0, 0)
